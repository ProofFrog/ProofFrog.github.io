...
- **`get_inlined_game`** -- returns the canonical form of an arbitrary game step expression without requiring the full proof to be parseable. Use this when writing intermediate games: it shows exactly what a game looks like after inlining, so you can write a matching `Game` definition. Also useful when the proof has stub reductions that prevent it from parsing cleanly.
