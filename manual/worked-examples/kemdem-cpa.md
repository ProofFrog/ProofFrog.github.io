...
The theorem proved here is: if {% katex %}K{% endkatex %} is CPA-secure, {% katex %}E{% endkatex %} has one-time secrecy (OTS), and {% katex %}E{% endkatex %}'s key generation algorithm produces uniformly distributed keys, then {% katex %}\mathsf{KEMDEM}(K,E){% endkatex %} is CPA-secure as a public key encryption scheme.
